...
void model_estimate(float *X_train_e, float *y_train_e, float *X_test_e, float *y_test_e, float *X_T_e, float *y_T_e, float *sprt_in, 
			...
			float *Bgd_out, float *rsd_out, float *R2_out, float *bic_out, int *roots, MPI_Comm comm_WORLD, MPI_Comm comm_NRND) {
    ...
    VectorXf btmp_r(n); MatrixXf btmp(nrnd, n);
    

    Map<MatrixXf> sprt_d (sprt_in, nMP, n);
    VectorXi sprt_ids(n);
    VectorXf Bgols_R2m(nbootE*nMP);
    MatrixXf Bgols_B(nbootE*nMP, n);	

    start_time = MPI_Wtime();
    for (int kk=0; kk<nbootE; kk++) {

	randomize(X_train_e, y_train_e, train, n, rank_NRND);
	...
	for (int mp=0; mp<nMP; mp++) {
		...
		if (rank_WORLD==0 && kk==0 && mp==0) {
			ofstream myfile121 ("rgstrct.dat");
		...
		/*Apply support*/
   		...
   		for (int i=0; i<zdids.size(); i++)
			...
			Bgols_R2m((kk*nMP)+mp) = my_Bgols_R2m; 
   	...
   	if (rank_NRND == 0) { 
		float v;
   		if (bgdOpt == 1) {
			...
			if (!ids_kk.isZero())
				btmp_r = Bgols_B.row(ids_kk(floor(ids_kk.size()/2)));
			else	
				btmp_r = Bgols_B.row(nbootE % nrnd);
		}
  ...
  *time = MPI_Wtime() - start_time;
  ...
  if (MPI_COMM_NULL != comm_root_nrnd) {
  	MPI_Gather (btmp_r.data(), n, MPI_FLOAT, btmp.data(), n, MPI_FLOAT, 0, comm_root_nrnd);
   }
