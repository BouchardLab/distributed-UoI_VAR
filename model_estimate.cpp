...
/*VectorXi where(VectorXi vec, float value) {
        vector<int> v_in;
...
void get_rows(float *mat_in, float *vec_in, float *mat_train, float *vec_train, float *mat_test, float *vec_test, int rows, int cols, int train, int test) {

        Map<MatrixXf> Mat_eig(mat_in, rows, cols);
        Map<VectorXf> Vec_eig(vec_in, rows);

        MatrixXf Mat_train, Mat_test;
        VectorXf Vec_train, Vec_test;

	Mat_train = Mat_eig.topRows(train);
	Vec_train = Vec_eig.head(train);
        Mat_test = Mat_eig.bottomRows(test);
        Vec_test = Vec_eig.tail(test);

        Map<MatrixXf> (mat_train, Mat_train.rows(), Mat_train.cols()) = Mat_train;
        Map<VectorXf> (vec_train, Vec_train.size()) = Vec_train;

	Map<MatrixXf> (mat_test, Mat_test.rows(), Mat_test.cols()) = Mat_test;
        Map<VectorXf> (vec_test, Vec_test.size()) = Vec_test;
}
...
void model_estimate(float *X_L_e, float *y_L_e, float *X_T_e, float *y_T_e, float *sprt_in, int L, int train, int m, int n, int n_rows, int k_rows, int nMP, int nbootE, int nrnd, double *time, 
    ...
    MPI_Group root_NRND;
    MPI_Group_incl(world_GROUP, nrnd, roots, &root_NRND);

    MPI_Comm comm_root_nrnd;
    MPI_Comm_create_group(comm_WORLD, root_NRND, 2, &comm_root_nrnd);
    ...
    VectorXf  R2_r(1), bic_r(1);
    VectorXf Bgd_r(n), rsd_r(T);
    MatrixXf btmp(nbootE, n);

    Map<MatrixXf> sprt_d (sprt_in, nMP, n);
    VectorXi sprt_ids(n);
    MatrixXf Bgols_R2m(nbootE,nMP);
    MatrixXf Bgols_B(nbootE*nMP, n);	 
    float *X_train_e,  *y_train_e, *X_test_e, *y_test_e;
    X_train_e =(float *) malloc (train * n * sizeof(float));
    y_train_e = (float *) malloc ( train * sizeof(float));
    X_test_e = (float *) malloc (test * n * sizeof(float));
    y_test_e = (float *)malloc(test * sizeof(float));

    //start_time = MPI_Wtime();
   /* estimate selected parameters using OLS with bagging */	
	int cc=0;
	randomize(X_L_e, y_L_e, L, n, cc);
    	for (int kk=0; kk<nbootE; kk++) {
		get_rows(X_L_e, y_L_e, X_train_e, y_train_e, X_test_e, y_test_e, L, n, train, test);		
		randomize(X_train_e, y_train_e, train, n, kk);  
		randomize(X_test_e, y_test_e, test, n, kk);	

		for (int mp=0; mp<nMP; mp++) {
			start_time = MPI_Wtime();
			_rgstrct = lasso(X_train_e, train, n, y_train_e, 0.0, comm_group); 
			*time = MPI_Wtime() - start_time;
			Map<VectorXf> rgstrct(_rgstrct, n);
    			VectorXi  zdids, arange(n);
 
    			sprt_ids = sprt_d.row(mp).unaryExpr(ptr_fun(not_NaN)).cast<int>(); 
			...
			free(_rgstrct);
    			...
    			if (rank_WORLD == 0) {
				Map<Matrix<float, Dynamic, Dynamic, RowMajor> > X_test (X_test_e, test, n);
				Map<VectorXf> y_test (y_test_e, test); 
				yhat = X_test * my_Bgols_B;
   				float r =(float) pearson1(yhat, y_test);
				float my_Bgols_R2m = r*r;
				Bgols_R2m(kk,mp) = my_Bgols_R2m; 
				Bgols_B.row((kk*nMP)+mp) = my_Bgols_B;
    			}
		}
   	} 
   	...
   	if (rank_WORLD == 0) {
		...
		Map<Matrix<float, Dynamic, Dynamic, RowMajor> > X_T(X_T_e, T, n);
        	...
        	//cout << "R2 = " << R2_r << endl;
        	rsd_r = y_T - yhat;
        	bic_r << (m-train) * log(rsd_r.dot(rsd_r)/(m-train)) + log(m-train) * n;
   	}

  /* free the memory for train and test models */
  ...
  /* Bcast result for parallel writing -- can be reduced if we can write only with Rank == 0. 
   ...
   */
   ...
   if (MPI_COMM_NULL != comm_root_nrnd) {
	...
	MPI_Bcast(rsd_r.data(), rsd_r.cols(), MPI_FLOAT, 0, comm_root_nrnd);
	MPI_Bcast(R2_r.data(), 1, MPI_FLOAT, 0, comm_root_nrnd);
	MPI_Bcast(bic_r.data(), 1, MPI_FLOAT, 0, comm_root_nrnd);
   }
   
  if (rank_NRND == 0) {
  	Map<VectorXf> (Bgd_out, Bgd_r.size()) = Bgd_r; 
 	Map<VectorXf> (rsd_out, rsd_r.size()) = rsd_r;
