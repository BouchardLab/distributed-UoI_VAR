...
int main(int argc, char** argv) {
  ...
  ************************/
  ...
  if (rank == 0) {
	B_select = (double *) malloc(nbootS * ncols * sizeof(double));
	R2m = (double *) malloc (nbootS * sizeof(double));
  }

  double *lamb;
  lamb = (double *)malloc (nbootS * sizeof(double)); 
  ...
  model_selection (X_train, y_train, X_test, ymean, lamb, B_select, R2m, &lasso1_end, &lasso2_end, nbootS, train_rows, ncols, qrows, MPI_COMM_WORLD, comm_g);
  ...
  double sel_end = MPI_Wtime() - select_t;
   if (rank == 0) {
	...
	printf("Lasso2 time : %f (s)\n", lasso2_end); 
        ...
        print_mat(B_select, nbootS, ncols, "B_select.dat"); 
	print_array(R2m, nbootS, "R2m.dat");
   }
...
************************/
  ...
  if (rank == 0)
        get_support (B_select, sprt, nbootS, ncols);
 
  if (rank == 0)
	print_mat(sprt, nbootS, ncols, "sprt.dat");
 

  if (rank == 0) {
	printf("passed support\n");
	print_mat(sprt, nbootS, ncols, "sprt.dat");
  }   
  ...
  if (MPI_COMM_NULL != comm_ROOT_NRND) {
	...
	MPI_Bcast(B_select, nbootS * ncols, MPI_DOUBLE, 0, comm_ROOT_NRND); 
	...
	write_selections(OUTPUTFILE1, B_select, R2m, lamb, sprt, nbootS, ncols, comm_ROOT_NRND);
